...
if ( defined( 'XKIT_BFI_THUMB_MODULE_ENABLE' ) && XKIT_BFI_THUMB_MODULE_ENABLE ) {
	 ...
	 */
	load_template( ABSPATH . WPINC . '/class-wp-image-editor.php', true );
	load_template( ABSPATH . WPINC . '/class-wp-image-editor-imagick.php', true );
	load_template( ABSPATH . WPINC . '/class-wp-image-editor-gd.php', true );
	...
	if ( ! function_exists( 'bfi_wp_image_editor_check_notice' ) ) {
		function bfi_wp_image_editor_check_notice() {
			printf( "<div class='error'><p>%s</div>",
				esc_html__( "The server does not have ImageMagick or GD installed and/or enabled! Any of these libraries are required for WordPress to be able to resize images. Please contact your server administrator to enable this before continuing." , 'xkit' ) );
		}
	}
	...
	if ( ! class_exists( 'BFI_Image_Editor_GD_1_3' ) ) {
		...
		class BFI_Image_Editor_GD_1_3 extends WP_Image_Editor_GD {
			...
			public function rotate( $angle ) {
				...
				return new WP_Error( 'image_rotate_error', esc_html__( 'Image rotate failed.', 'xkit' ), $this->file );
			}
			...
			public function opacity( $opacity ) {
				...
				return new WP_Error( 'image_opacity_error', esc_html__('Image opacity change failed.', 'xkit' ), $this->file );
			}
			...
			public function colorize( $hexColor ) {
				...
				return new WP_Error( 'image_colorize_error', esc_html__( 'Image color change failed.', 'xkit' ), $this->file );
			}
			...
			public function grayscale() {
				...
				return new WP_Error( 'image_grayscale_error', esc_html__( 'Image grayscale failed.', 'xkit' ), $this->file );
			}
			...
			public function negate() {
				...
				return new WP_Error( 'image_negate_error', esc_html__( 'Image negate failed.', 'xkit' ), $this->file );
			}
		}
	}
