...
class Xkit_TheLib_1_0_17 {
	...
	protected function _sess_add( $key, $value ) {
		if ( ! isset( $_SESSION[ '_lib_persist_' . $key ] ) || ! is_array( $_SESSION[ '_lib_persist_' . $key ] ) ) {
	...
	protected function _css_url( $file ) {
		...
		if ( null === $Url ) {
			$Url = get_template_directory_uri() . '/framework/modules/sidebar-generator/inc/external/wpmu-lib/css/';
		}
		return $Url . $file;
	}
	...
	protected function _js_url( $file ) {
		...
		if ( null === $Url ) {
			$Url = get_template_directory_uri() . '/framework/modules/sidebar-generator/inc/external/wpmu-lib/js/';
		}
		return $Url . $file;
	}
	...
	protected function _include_path( $file ) {
		static $Path = null;
		if ( null === $Path ) {
			$Path = get_template_directory() . '/framework/modules/sidebar-generator/inc/external/wpmu-lib/inc/';
		}
		return $Path . $file;
	}
	...
	/**
	 * Action handler for after_setup_theme. This decides if the pointer will be displayed.
	 *
	 * @since  1.0.2
	 * @private
	 */
	...
	public function _pointer_print_scripts() {
		...
		foreach ( $items as $item ) {
			...
			$xkit_cs_pointer_item = $item;
			load_template( $this->_include_path( 'pointer.php' ) );
		}
	}
	...
	public function _admin_notice_callback() {
		...
		foreach ( $items as $item ) {
			...
			if ( empty( $screen ) || $screen_id == $screen ) {
				echo '<div class="' . esc_attr( $class ) . '"><p>' . wp_kses( $text, 'post' ) . '</p></div>';
			}
		}
	}
	...
	/**
	 * Short function for Xkit_WDev()->load_fields
	 *
	 * @since  1.0.14
	...
	/**
	 * Short function for Xkit_WDev()->load_fields( $_REQUEST, ... )
	 *
	 * @since  1.0.14
	 * @param  strings|Array <param list>
	 * @return int Number of missing fields that were initialized.
