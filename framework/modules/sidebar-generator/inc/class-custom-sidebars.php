...
load_template( get_template_directory() . '/framework/modules/sidebar-generator/inc/class-custom-sidebars-widgets.php', true );
load_template( get_template_directory() . '/framework/modules/sidebar-generator/inc/class-custom-sidebars-editor.php', true );
load_template( get_template_directory() . '/framework/modules/sidebar-generator/inc/class-custom-sidebars-replacer.php', true );
load_template( get_template_directory() . '/framework/modules/sidebar-generator/inc/class-custom-sidebars-explain.php', true );


/**
 * Main plugin file.
 * The Xkit_CustomSidebars class encapsulates all our plugin logic.
 */
class Xkit_CustomSidebars {
	/**
	...
	static public function instance() {
		...
		// The Xkit_WDev()->pointer() notification is based on current user...
	...
	private function __construct() {
		 ...
		 *
		 * ========== Pointer ==========
		 *  Internal ID:  wpmudcs1 [WPMUDev Xkit_CustomSidebars 1]
		...
		Xkit_WDev()->pointer(
			...
			esc_html__( 'Custom Sidebars', 'xkit' ),    // Title
			sprintf(
				wp_kses_post( __(	'Now you can create and edit custom sidebars in your <a href="%1$s">Widgets screen</a>!', 'xkit' ) ),
		...
		*/
		...
		if ( true === self::$accessibility_mode ) {
			Xkit_WDev()->message(
				sprintf(
					wp_kses_post( __( '<strong>Accessibility mode is not supported by the %1$s plugin.</strong><br /><a href="%2$s">Click here</a> to disable accessibility mode and use the %1$s plugin!', 'xkit' ) ),
					'Custom Sidebars',
			...
			Xkit_WDev()->add_ui( 'core', 'widgets.php' );
			Xkit_WDev()->add_ui( 'scrollbar', 'widgets.php' );
			Xkit_WDev()->add_ui( 'select', 'widgets.php' );
			Xkit_WDev()->add_ui( get_template_directory_uri() . '/framework/modules/sidebar-generator/js/cs.js', 'widgets.php' );
			Xkit_WDev()->add_ui( get_template_directory_uri() . '/framework/modules/sidebar-generator/css/cs.css', 'widgets.php' );

			// AJAX actions
			add_action( 'wp_ajax_cs-ajax', array( $this, 'ajax_handler' ) );

			// Extensions use this hook to initialize themselfs.
			do_action( 'xkit_cs_init' );
			...
			if ( ! empty( $_GET['cs-msg'] ) ) {
				...
				$msg = wp_kses( $_GET['cs-msg'], $kses_args );

				if ( ! empty( $msg ) ) {
					Xkit_WDev()->message( $msg );
				}
			}
	...
	static public function supported_post_type( $posttype ) {
		...
		if ( ! isset( $Response[ $posttype ] ) ) {
			 ...
			 */
			$response = apply_filters( 'xkit_cs_support_posttype', $response, $posttype );
	...
	public function ajax_handler() {
		...
		if( isset( $_GET['do'] ) ){
			$get_action = sanitize_text_field( $_GET['do'] );
		}
		 ...
		 */
		do_action( 'xkit_cs_ajax_request', $action );
		 ...
		 * @param  string $action The specified ajax action.
		 */
		do_action( 'xkit_cs_ajax_request_get', $get_action );
	}
};
