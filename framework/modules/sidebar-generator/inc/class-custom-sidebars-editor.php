...
add_action( 'xkit_cs_init', array( 'Xkit_CustomSidebarsEditor', 'instance' ) );

/**
 * Provides all the functionality for editing sidebars on the widgets page.
 */
class Xkit_CustomSidebarsEditor extends Xkit_CustomSidebars {
	...
	public static function instance() {
		...
		if ( null === $Inst ) {
			$Inst = new Xkit_CustomSidebarsEditor();
		}
	...
	public function handle_ajax( $action ) {
		...
		$sb_id = '';
		if( isset( $_POST['sb'] ) ){
			$sb_id = sanitize_text_field( $_POST['sb'] );
		}
		...
		if ( ! current_user_can( self::$cap_required ) ) {
			$req = self::req_err(
				$req,
				esc_html__( 'You do not have permission for this', 'xkit' )
			);
		} else {
			switch ( $action ) {
		...
		if ( $is_json ) {
			...
			ob_start();
			load_template( get_template_directory() . '/framework/modules/sidebar-generator/views/' . $view_file );
			$resp = ob_get_clean();
	...
	private function save_item( $req, $data )  {
		...
		if( isset( $_POST['description'] ) ){
			$sb_desc = wp_kses_post( stripslashes( trim( $_POST['description'] ) ) );
		}
		...
		if ( empty( $sb_name ) ) {
			return self::req_err(
				$req,
				esc_html__( 'Sidebar-name cannot be empty', 'xkit' )
			);
		}
		...
		if ( empty( $sb_id ) ) {
			...
			if ( ! $sidebar ) {
				return self::req_err(
					$req,
					esc_html__( 'The sidebar does not exist', 'xkit' )
				);
			}
		}
		...
		if( isset( $_POST['before_widget'] ) ){
			$sidebar['before_widget'] = wp_kses_post( stripslashes( trim( $_POST['before_widget'] ) ) );
		}
		if( isset( $_POST['after_widget'] ) ){
			$sidebar['after_widget'] = wp_kses_post( stripslashes( trim( $_POST['after_widget'] ) ) );
		}
		if( isset( $_POST['before_title'] ) ){
			$sidebar['before_title'] = wp_kses_post( stripslashes( trim( $_POST['before_title'] ) ) );
		}
		if( isset( $_POST['after_title'] ) ){
			$sidebar['after_title'] = wp_kses_post( stripslashes( trim( $_POST['after_title'] ) ) );
		}

		if ( $action == 'insert' ) {
			$sidebars[] = $sidebar;
			$req->message = sprintf(
				wp_kses_post( __( 'Created new sidebar <strong>%1$s</strong>', 'xkit' ) ),
			...
			foreach ( $sidebars as $ind => $item ) {
				if ( $item['id'] == $sb_id ) {
					$req->message = sprintf(
						wp_kses_post( __( 'Updated sidebar <strong>%1$s</strong>'  , 'xkit' ) ),
						esc_html( $sidebar['name'] )
					);
			...
			if ( ! $found ) {
				return self::req_err(
					$req,
					esc_html__( 'The sidebar was not found', 'xkit' )
				);
			}
		}
	...
	private function delete_item( $req ) {
		...
		foreach ( $sidebars as $ind => $item ) {
			if ( $item['id'] == $req->id ) {
				$found = true;
				$req->message = sprintf(
					wp_kses_post( __( 'Deleted sidebar <strong>%1$s</strong>', 'xkit' ) ),
	...
	private function get_location_data( $req ) {
		...
		$archive_type = array(
			'_blog' => esc_html__( 'Front Page', 'xkit' ),
			'_search' => esc_html__( 'Search Results', 'xkit' ),
			'_404' => esc_html__( 'Not found (404)', 'xkit' ),
			'_authors' => esc_html__( 'Any Author Archive', 'xkit' ),
			'_tags' => esc_html__( 'Tag Archives', 'xkit' ),
			'_date' => esc_html__( 'Date Archives', 'xkit' ),
		);
		...
		foreach ( $raw_posttype as $item ) {
			$sel_single = '';
			if( isset( $defaults['post_type_single'][$item->name] ) ){
		...
		$category_single = '';
		$category_archive = '';
		if( isset( $defaults['category_single']['all'] ) ){
			$category_single = $defaults['category_single']['all'];
		}
		if( isset( $defaults['category_archive']['all'] ) ){
			$category_archive = $defaults['category_archive']['all'];
		}
		$custom_categories['all'] = array(
			'name' 		=> esc_html__( 'For all categories', 'xkit' ),
			'count' 	=> $count_posts,
			'single' 	=> self::get_array( $category_single ),
			'archive' 	=> self::get_array( $category_archive ),
		);
		...
		foreach ( $raw_cat as $item ) {
			$sel_single = '';
			$sel_archive = '';
			if( isset( $defaults['category_single'][$item->term_id] ) ){
				$sel_single = $defaults['category_single'][$item->term_id];
			}
			if( isset( $defaults['category_archive'][$item->term_id] ) ){
		...
		// Sort categories
		$sorted_cats = $custom_categories;
		foreach( $categories as $cat_key => $cat_val ){
			$sorted_cats[$cat_key] = $cat_val;
		}
		...
		foreach ( $raw_posttype as $item ) {
			if ( $item->name == 'post' ) {
				$label = esc_html__( 'Post Index', 'xkit' );
			} else {
				if ( ! $item->has_archive ) { continue; }
				$label = sprintf(
					esc_html__( '%1$s Archives', 'xkit' ),
					$item->labels->singular_name
				);
			}
			
			$sel_archive = '';
			if( isset( $defaults['post_type_archive'][$item->name] ) ){
		...
		foreach ( $archive_type as $key => $name ) {
			$sel_archive = '';
			if( isset( $defaults[ substr( $key, 1 ) ] ) ){
	...
	private function set_location_data( $req ) {
		...
		foreach ( $sidebars as $sb_id ) {
			...
			// Category settings for all categories.
			$all_name = 'all'; 
			if (
				is_array( $data['cat'][$sb_id] ) &&
				in_array( $all_name, $data['cat'][$sb_id] )
			) {
				$options['category_single'][$all_name][$sb_id] = $req->id;
			} else
			if (
				isset( $options['category_single'][$all_name][$sb_id] ) &&
				$options['category_single'][$all_name][$sb_id] == $req->id
			) {
				unset( $options['category_single'][$all_name][$sb_id] );
			}

			if (
				is_array( $data['arc-cat'][$sb_id] ) &&
				in_array( $all_name, $data['arc-cat'][$sb_id] )
			) {
				$options['category_archive'][$all_name][$sb_id] = $req->id;
			} else
			if (
				isset( $options['category_archive'][$all_name][$sb_id] ) &&
				$options['category_archive'][$all_name][$sb_id] == $req->id
			) {
				unset( $options['category_archive'][$all_name][$sb_id] );
			}
		...
		$req->message = sprintf(
			wp_kses_post( __( 'Updated sidebar <strong>%1$s</strong> settings.', 'xkit' ) ),
			esc_html( $req->sidebar['name'] )
		);
		self::set_options( $options );
		return $req;
	...
	protected function print_sidebars_form( $post_id, $type = 'metabox' ) {
		global $wp_registered_sidebars, $xkit_selected_sidebars;
		...
		$xkit_selected_sidebars = array();
		if ( ! empty( $sidebars ) ) {
			foreach ( $sidebars as $s ) {
				if ( isset( $replacements[$s] ) ) {
					$xkit_selected_sidebars[$s] = $replacements[$s];
				} else {
					$xkit_selected_sidebars[$s] = '';
				}
			}
		}
		load_template( get_template_directory() . '/framework/modules/sidebar-generator/views/metabox.php' );
	}
		...
		if ( ! empty( $sidebars ) ) {
			foreach ( $sidebars as $sb_id ) {
				if ( isset( $_POST[ 'cs_replacement_' . $sb_id ] ) ) {
					$replacement = sanitize_text_field( $_POST[ 'cs_replacement_' . $sb_id ] );
