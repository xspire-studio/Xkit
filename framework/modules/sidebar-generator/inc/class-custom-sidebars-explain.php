...
add_action( 'xkit_cs_init', array( 'Xkit_CustomSidebarsExplain', 'instance' ) );
 ...
 *
 */
class Xkit_CustomSidebarsExplain extends Xkit_CustomSidebars {

	/**
	 * Infos added via xkit_cs_explain.
	 * @var array
	 */
	private $infos = array();
	...
	public static function instance() {
		...
		if ( null === $Inst ) {
			$Inst = new Xkit_CustomSidebarsExplain();
		}
	...
	/**
	 * Called by action 'xkit_cs_widget_header'. Output the export/import button in
	 * the widget header.
	 *
	 * @since  2.0.9.1
	 */
	public function widget_header() {
		...
		<a href="#"
			...
			data-label-off="<?php esc_html_e( 'Show explanations', 'xkit' ); ?>"
			data-label-on="<?php esc_html_e( 'Hide explanations', 'xkit' ); ?>">
		</a>
		<?php
		*/
	}
		...
		<div class="cs-infos" style="width:600px;margin:10px auto;padding:10px;color:#666;background:#FFF;">
			...
			<ul>
				<?php foreach ( $this->infos as $info ) : ?>
					<li><?php echo wp_kses( $info, 'post' ); ?></li>
				<?php endforeach; ?>
			</ul>
		</div>
		<?php
	...
	public function before_sidebar( $index, $has_widgets ) {
		...
		if( isset( $wp_registered_sidebars[$index]['before_widget'] ) ){
			$before_widget = $wp_registered_sidebars[$index]['before_widget'];
		}
		if( isset( $wp_registered_sidebars[$index]['after_widget'] ) ){
			$after_widget = $wp_registered_sidebars[$index]['after_widget'];
		}
