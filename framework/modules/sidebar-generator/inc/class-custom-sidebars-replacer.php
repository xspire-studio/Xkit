...
add_action( 'xkit_cs_init', array( 'Xkit_CustomSidebarsReplacer', 'instance' ) );
 ...
 * @since  2.0
 */
class Xkit_CustomSidebarsReplacer extends Xkit_CustomSidebars {
	...
	public static function instance() {
		...
		if ( null === $Inst ) {
			$Inst = new Xkit_CustomSidebarsReplacer();
		}
	...
	public function register_custom_sidebars() {
		...
		foreach ( $sb as $sidebar ) {
			 ...
			 *           the sidebar.
			 */
			$sidebar = apply_filters( 'xkit_cs_sidebar_params', $sidebar );
	...
	public function replace_sidebars() {
		...
		$expl = Xkit_CustomSidebarsExplain::do_explain();

		$expl && do_action( 'xkit_cs_explain', '<h4>Replace sidebars</h4>', true );

		do_action( 'xkit_cs_before_replace_sidebars' );
		 ...
		 */
		do_action( 'xkit_cs_predetermine_replacements', $defaults );

		// Legacy handler with camelCase
		do_action( 'xkit_cs_predetermineReplacements', $defaults );
		...
		foreach ( $replacements as $sb_id => $replace_info ) {
			if ( ! is_array( $replace_info ) || count( $replace_info ) < 3 ) {
				$expl && do_action( 'xkit_cs_explain', 'Replacement for "' . $sb_id . '": -none-' );
				continue;
			}
			
			if( !isset( $replace_info[0] ) ){
				$replace_info[0] = '';
			}
			if( !isset( $replace_info[1] ) ){
				$replace_info[1] = '';
			}
			if( !isset( $replace_info[2] ) ){
				$replace_info[2] = '';
			}
			...
			if ( $check ) {
				$expl && do_action( 'xkit_cs_explain', 'Replacement for "' . $sb_id . '": ' . $replacement );
	...
	public function determine_replacements( $options ) {
		...
		if ( is_single() ) {
			...
			$expl && do_action( 'xkit_cs_explain', 'Type 1: Single ' . ucfirst( $post_type ) );

			if ( ! self::supported_post_type( $post_type ) ) {
				$expl && do_action( 'xkit_cs_explain', 'Invalid post type, use default sidebars.' );
				return $options;
			}
			...
			if ( $replacements_todo > 0 ) {
				...
				while ( $replacements_todo > 0 && $ind >= 0 ) {
					...
					foreach ( $sidebars as $sb_id ) {
						...
						if( isset( $options['category_single']['all'][$sb_id] ) ){
							$replacements[$sb_id] = array(
								$options['category_single']['all'][$sb_id],
								'category_single',
								'all',
							);
							$replacements_todo -= 1;
						}
						elseif ( ! empty( $options['category_single'][$cat_id][$sb_id] ) ) {
							$replacements[$sb_id] = array(
		...
		// 2 |== Category archive ----------------------------------------------
		if ( is_category() ) {
			$expl && do_action( 'xkit_cs_explain', 'Type 2: Category Archive' );
			...
			while ( $current_category != 0 && $replacements_todo > 0 ) {
				foreach ( $sidebars as $sb_id ) {
					...
					if( isset( $options['category_archive']['all'][$sb_id] ) ){
						$replacements[$sb_id] = array(
							$options['category_archive']['all'][$sb_id],
							'category_archive',
							'all',
						);
						$replacements_todo -= 1;
					}
					elseif ( ! empty( $options['category_archive'][$current_category][$sb_id] ) ) {
						$replacements[$sb_id] = array(
		...
		if ( is_search() ) {
			$expl && do_action( 'xkit_cs_explain', 'Type 3: Search Results' );
		...
		if ( ! is_category() && ! is_singular() && get_post_type() != 'post' ) {
			$post_type = get_post_type();
			$expl && do_action( 'xkit_cs_explain', 'Type 4: ' . ucfirst( $post_type ) . ' Archive' );
		...
		if ( is_page() && ! is_front_page() ) {
			$post_type = get_post_type();
			$expl && do_action( 'xkit_cs_explain', 'Type 5: ' . ucfirst( $post_type ) );
		...
		if ( is_front_page() && is_page() ) {
			...
			$expl && do_action( 'xkit_cs_explain', 'Type 6: Front Page' );
		...
		if ( is_home() && !is_page() ) {
			...
			$expl && do_action( 'xkit_cs_explain', 'Type 7: Post Index' );
		...
		// 8 |== Tag archive ---------------------------------------------------
		if ( is_tag() ) {
			$expl && do_action( 'xkit_cs_explain', 'Type 8: Tag Archive' );
		...
		if ( is_author() ) {
			...
			$expl && do_action( 'xkit_cs_explain', 'Type 9: Author Archive' );
		...
		// 10 |== Date archive -------------------------------------------------
		if ( is_date() ) {
			$expl && do_action( 'xkit_cs_explain', 'Type 10: Date Archive' );
		...
		// 11 |== 404 not found ------------------------------------------------
		if ( is_404() ) {
			$expl && do_action( 'xkit_cs_explain', 'Type 11: 404 not found' );
		 ...
		 */
		$replacements = apply_filters( 'xkit_cs_replace_sidebars', $replacements );
